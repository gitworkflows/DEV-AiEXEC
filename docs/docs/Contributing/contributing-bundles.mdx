...
Follow these steps to add components to the **Bundles** section of the **Components** menu in the Aiexec visual editor.

Example adds a new bundle named `DarthVader`.

## Add the bundle to the backend folder

1. Navigate to the backend directory in the Aiexec repository and create a new folder for your bundle.
...
1. Navigate to the frontend directory in the Aiexec repository to add your bundle's icon.
...
8. To add your bundle to the **Bundles** menu, edit the [`SIDEBAR_BUNDLES` array](https://github.com/khulnasoft-lab/aiexec/blob/main/src/frontend/src/utils/styleUtils.ts#L231) in `/src/frontend/src/utils/styleUtils.ts`.

    Add an object to the array with the following keys:

    * `display_name`: The text label shown in the Aiexec visual editor
    * `name`: The name of the folder you created within the `/src/backend/base/aiexec/components` directory
