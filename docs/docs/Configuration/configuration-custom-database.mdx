...
This guide walks you through setting up an external database for Aiexec by replacing the default SQLite connection string `sqlite:///./aiexec.db` with PostgreSQL, both in local and containerized environments.

In this configuration, all structured application data from Aiexec, including flows, message history, and logs, is instead managed by PostgreSQL.
...
## Connect Aiexec to a local PostgreSQL database

1. If Aiexec is running, stop Aiexec with <kbd>Ctrl+C</kbd>.
...
3. Create a Aiexec `.env` file:

    ```
    touch .env
    ```

    You can use the [`.env.example`](https://github.com/khulnasoft-lab/aiexec/blob/main/.env.example) file in the Aiexec repository as a template for your own `.env` file.

4. In your `.env` file, set `AIEXEC_DATABASE_URL` to your PostgreSQL connection string:

    ```text
    AIEXEC_DATABASE_URL="postgresql://user:password@localhost:5432/dbname"
    ```

5. Save your changes, and then start Aiexec with your `.env` file:

    ```bash
    uv run aiexec run --env-file .env
    ```

6. In Aiexec, run any flow to create traffic.
...
## Deploy Aiexec and PostgreSQL containers with docker-compose.yml

Launching Aiexec and PostgreSQL containers in the same Docker network ensures proper connectivity between services.
For an example, see the [`docker-compose.yml`](https://github.com/khulnasoft-lab/aiexec/blob/main/docker_example/docker-compose.yml) file in the Aiexec repository.

The configuration in the example `docker-compose.yml` also sets up persistent volumes for both Aiexec and PostgreSQL data.
...
In contrast, if you run PostgreSQL separately with `docker run`, it launches in a different network than the Aiexec container, and this prevents Aiexec from connecting to PostgreSQL using the service name.

To start the Aiexec and PostgreSQL services with the example Docker Compose file, navigate to the `aiexec/docker_example` directory, and then run `docker-compose up`.
...
## Deploy multiple Aiexec instances with a shared PostgreSQL database
...
This example populates the values in `docker-compose.yml` with values from your Aiexec `.env` file.
...
For example:
    ...
    services:
      ...
      aiexec-1:
        image: khulnasoft/aiexec:latest
        pull_policy: always
        ports:
          - "${AIEXEC_PORT_1}:7860"
        depends_on:
...
* [Aiexec database guide for enterprise DBAs](/enterprise-database-guide)
