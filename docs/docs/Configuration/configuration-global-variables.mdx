...
:::warning
Deleting a global variable permanently deletes any references to it from all flows in your Aiexec projects.
:::
...
If you want to automatically set fallback values for your global variables from environment variables, set `AIEXEC_FALLBACK_FROM_ENV_VAR=True` in your `.env` file. When this setting is enabled, if a global variable isn't found, Aiexec attempts to use an environment variable with the same name as a backup.
