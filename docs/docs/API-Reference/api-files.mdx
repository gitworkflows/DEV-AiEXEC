...
Use the `/files` endpoints to move files between your local machine and Aiexec.
...
- `/v2` files are tracked in the Aiexec database.
...
Send image files to Aiexec to use them in flows.

The default file limit is 100 MB.
To change this limit, set the `AIEXEC_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).

1. Attach the image to a `POST /v1/files/upload/$FLOW_ID` request with `--form` (`-F`) and the file path:

    ```bash
    curl -X POST "$AIEXEC_URL/api/v1/files/upload/$FLOW_ID" \
      -H "Content-Type: multipart/form-data" \
      -H "x-api-key: $AIEXEC_API_KEY" \
      -F "file=@PATH/TO/FILE.png"
    ```

    A successful request returns the `file_path` for the image in the Aiexec file management system in the format `FLOW_ID/TIMESTAMP_FILENAME.TYPE`.
    For example:
...
To create a Aiexec API key and export it as an environment variable, see [Get started with the Aiexec API](/api-reference-api-examples).
...
This endpoint uploads files to your Aiexec server's file management system.
To use an uploaded file in a flow, send the file path to a flow with a [**File** component](/components-data#file).

The default file limit is 100 MB. To configure this value, change the `AIEXEC_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).
...
This component loads files into flows from your local machine or Aiexec file management.
