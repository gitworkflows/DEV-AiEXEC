...
Aiexec flows are fully serializable and can be saved and loaded from the file system where Aiexec is installed.

:::tip
To try building and running a flow in a few minutes, see the [Aiexec quickstart](/get-started-quickstart).
:::
...
* **Create a flow from a template**: Select a project, click **New Flow**, and then click the template that you want to use.
    ...
    You can also [contribute templates](/contributing-templates) to the Aiexec codebase.
...
You can also create a flow with the [Aiexec API](/api-flows), but the Aiexec team recommends using the [visual editor](/concepts-overview) until you are familiar with flow creation.
...
When you're ready to use Aiexec for application development, learn how to [trigger flows with the Aiexec API](/concepts-publish), explore more advanced configuration options like [custom dependencies](/install-custom-dependencies), and, eventually, [containerize your Aiexec application](/develop-application).

When you're ready to go to production or deploy a Aiexec MCP server for access over the public internet, see [Aiexec deployment overview](/deployment-overview).

#### Flow graphs

When a flow runs, Aiexec builds a Directed Acyclic Graph (DAG) object from the nodes (components) and edges (connections), and the nodes are sorted to determine the order of execution.
...
The **Projects** page is where you arrive when you launch Aiexec.
From here, you can manage flows and your projects' [MCP servers](/mcp-server).

Aiexec projects are like folders that you can use to organize related flows.
...
:::tip
To get back to the **Projects** page after editing a flow, click the project name or Aiexec icon in the Aiexec header.
:::
...
- **macOS Desktop**: `/Users/<username>/.aiexec/cache`
- **Windows Desktop**: `C:\Users\<username>\AppData\Roaming\com.Aiexec\cache`
- **OSS macOS/Windows/Linux/WSL (uv pip install)**: `<path_to_venv>/lib/python3.12/site-packages/aiexec/cache`
- **OSS macOS/Windows/Linux/WSL (git clone)**: `<path_to_clone>/src/backend/base/aiexec/cache`

The overall storage location can be customized with the `AIEXEC_CONFIG_DIR` environment variable, and the flow log storage location can be customized separately with the `AIEXEC_LOG_FILE` environment variable.
...
* [Aiexec environment variables](/environment-variables)