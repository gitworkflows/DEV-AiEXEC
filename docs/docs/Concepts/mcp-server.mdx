---
title: Use Aiexec as an MCP server
slug: /mcp-server
---
...
This page describes how to use Aiexec as an MCP server that exposes your flows as [tools](https://modelcontextprotocol.io/docs/concepts/tools) that [MCP clients](https://modelcontextprotocol.io/clients) can use when generating responses.

For information about using Aiexec as an MCP client and managing MCP server connections within flows, see [Use Aiexec as an MCP client](/mcp-client).

## Prerequisites

* A [Aiexec project](/concepts-flows#projects) with at least one flow that has a [**Chat Output** component](/components-io#chat-output).
...
* [ngrok installed](https://ngrok.com/docs/getting-started/#1-install-ngrok) and an [ngrok authtoken](https://dashboard.ngrok.com/get-started/your-authtoken) if you want to [deploy a public Aiexec server](/deployment-public-server).
...
Each [Aiexec project](/concepts-flows#projects) has an MCP server that exposes the project's flows as tools for use by MCP clients.
...
To edit the names and descriptions of flow tools on a Aiexec MCP server, do the following:
...
Because MCP clients treat your Aiexec project as a single MCP server with all of your enabled flows listed as tools, unclear names and descriptions can cause the agent to select tools incorrectly or inconsistently.
...
To provide more context about your flows, make sure to name and describe your flows clearly when configuring your Aiexec project's MCP server.
...
After connecting your Aiexec MCP server to Cursor, you can ask Cursor about the resume, such as `What job experience does Emily have?`.
...
## Connect clients to your Aiexec MCP server {#connect-clients-to-use-the-servers-actions}

Aiexec provides automatic installation and code snippets to help you deploy your Aiexec MCP servers to your local MCP clients.

<Tabs>
<TabItem value="auto" label="Auto install" default>

:::info
The auto install option is available only for specific MCP clients, and the client must be installed locally to your Aiexec server.
If your client isn't supported or installed remotely from your Aiexec server, use the **JSON** option.
:::

1. Install [Cursor](https://docs.cursor.com/get-started/installation), Claude, or Windsurf local to your Aiexec instance.

2. Recommended: Configure [authentication](#authentication) for your MCP server.

3. In Aiexec, on the **Projects** page, click the **MCP Server** tab.

4. On the **Auto install** tab, find your MCP client provider, and then click <Icon name="Plus" aria-hidden="true"/> **Add**.

Your Aiexec project's MCP server is automatically added to the configuration file for your local Cursor, Claude, or Windsurf client.
For example, with Cursor, the server configuration is added to the `mcp.json` configuration file.

Aiexec attempts to add this configuration even if the selected client isn't installed.
To verify the installation, check the available MCP servers in your client.

</TabItem>
<TabItem value="JSON" label="JSON">

The JSON option allows you to connect a Aiexec MCP server to any local or remote MCP client.
...
4. In Aiexec, on the **Projects** page, click the **MCP Server** tab.
...
For example:
    ...
    The **MCP Server** tab automatically populates the `PROJECT_NAME`, `AIEXEC_SERVER_ADDRESS`, and `PROJECT_ID` values.

    The default Aiexec server address is `http://localhost:7860`.
    If you are using a [public Aiexec server](/deployment-public-server), the server address is automatically included.

    If your Aiexec server requires authentication, you must include your Aiexec API key in the configuration.
...
7. Confirm that your Aiexec MCP server is on Cursor's **MCP Servers** list.

</TabItem>
</Tabs>

Once your MCP client is connected to your Aiexec project's MCP server, your flows are registered as tools.
...
You must provide a Aiexec API key in your MCP client configuration if your [Aiexec server has authentication enabled](/api-keys-and-authentication#start-a-aiexec-server-with-authentication-enabled).
...
### Deploy your Aiexec MCP server externally {#deploy-your-server-externally}

To deploy your Aiexec MCP server externally, see [Deploy a public Aiexec server](/deployment-public-server).
...
3. In the MCP Inspector UI, enter the connection details for your Aiexec project's MCP server:
    ...
    - **URL**: Enter the Aiexec MCP server's `sse` endpoint. For example: `http://localhost:7860/api/v1/mcp/project/d359cbd4-6fa2-4002-9d53-fa05c645319c/sse`
...
## Troubleshoot Aiexec MCP servers {#troubleshooting-mcp-server}
...
- [Use Aiexec as an MCP client](/mcp-client)
