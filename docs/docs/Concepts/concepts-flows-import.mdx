...
You can export flows to transfer them between Aiexec instances, share them with others, or create backups.
...
* **Export with the Aiexec API**: To export one flow, use the [`/flows/download`](/api-flows#export-flows) endpoint.
...
For more information, see [Aiexec JSON file contents](#aiexec-json-file-contents).
...
If your key is stored in a Aiexec global variable, **Save with my API keys** exports only the variable name.
:::

When you or another user import the flow to another Aiexec instance, that instance must have Aiexec global variables with the same names and valid values in order to run the flow successfully.
If any variables are missing or invalid, those variables must be created or edited after importing the flow.

## Import a flow

You can import Aiexec JSON files from your local machine in the following ways:

* **Import to projects**: On the **Projects** page, click <Icon name="Upload" aria-hidden="true"/> **Upload a flow**, and then select the Aiexec JSON file to import.
* **Import anywhere**: Drag and drop Aiexec JSON files from your file explorer into your Aiexec window to import a flow from any Aiexec page.
* **Import with the Aiexec API**: To import one Aiexec JSON file, use the [`/flows/upload/`](/api-flows#import-flows) endpoint.
To import a zip archive of Aiexec JSON files, use the [`/projects/upload`](/api-projects#import-a-project) endpoint.

### Run an imported flow

Once imported, your flow is ready to use.
If the flow contains any global variables, make sure your Aiexec instance has global variables with the same names and valid values.
For more information, see [Save with my API keys](/concepts-flows-import#save-with-my-api-keys).

## Aiexec JSON file contents

An exported flow is downloaded to your local machine as a JSON file named `FLOW_NAME.json`.

Aiexec JSON files contain [nodes](#nodes) and [edges](#edges) that describe components and connections, and [additional metadata](#additional-metadata-and-project-information) that describe the flow.

For an example Aiexec JSON file, examine the [Basic Prompting.json](https://github.com/khulnasoft-lab/aiexec/blob/main/src/backend/base/aiexec/initial_setup/starter_projects/Basic%20Prompting.json) file in the Aiexec repository.
