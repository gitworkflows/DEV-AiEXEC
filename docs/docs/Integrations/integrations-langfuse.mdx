...
- A [running Aiexec server](/get-started-installation) with a [flow](/concepts-flows) that you want to trace

:::tip
If you need a flow to test the Langfuse integration, see the [Aiexec quickstart](/get-started-quickstart).
:::
...
1. Start Aiexec in the same environment where you set the Langfuse environment variables:
...
To disable the Langfuse integration, remove the [Langfuse environment variables](#langfuse-credentials), and then restart Aiexec.
...
3. Add your Aiexec credentials to your Aiexec `docker-compose.yml` file in the `environment` section.

    The following example is based on the [example `docker-compose.yml`](https://github.com/khulnasoft-lab/aiexec/blob/main/docker_example/docker-compose.yml).

    ```yml
    services:
      aiexec:
        image: khulnasoft/aiexec:latest # or another version tag on https://hub.docker.com/r/khulnasoft/aiexec
...
5. To confirm Langfuse is connected to your Aiexec container, run the following command:
