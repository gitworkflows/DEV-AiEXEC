...
Aiexec provides flexible memory management options for storage and retrieval of data relevant to your flows and your Aiexec server.
This includes essential Aiexec database tables, file management, and caching, as well as chat memory.

## Storage options and paths

Aiexec supports both local memory and external memory options.

Aiexec's default storage option is a [SQLite](https://www.sqlite.org/) database stored in your system's cache directory.
...
- **Windows Desktop**: `C:\Users\<name>\AppData\Roaming\com.Aiexec\data\aiexec.db`
...
Alternatively, you can use an external PostgreSQL database for all of your Aiexec storage.
You can also selectively use external storage for chat memory, separate from other Aiexec storage.
...
• **ApiKey**: Manages Aiexec API authentication keys. Component API keys are stored in the **Variables** table. For more information, see [API keys and authentication](/api-keys-and-authentication).
...
To replace the default Aiexec SQLite database with another database, modify the `AIEXEC_DATABASE_URL` environment variable, and then start Aiexec with your `.env` file:
...
For other options, see the `AIEXEC_CACHE_TYPE` [environment variable](/environment-variables).

## Store chat memory

Chat-based flows with a **Chat Input** or **Chat Output** component produce chat history that is stored in the Aiexec `messages` table.
...
All messages are stored in [Aiexec storage](#storage-options-and-paths), and the component provides minimal memory configuration options, such as the number of messages to retrieve.
...
* **Message History component**: By default, this component stores and retrieves memories from Aiexec storage, unless you attach a third-party chat memory component. It provides a few more options for sorting and filtering memories, although most of these options are also built-in to the **Agent** component as configurable or fixed parameters.
...
Typically, this is necessary only if you have specific storage needs that aren't met by Aiexec storage.
For example, if you want to manage chat memory data by directly working with the database, or if you want to use a different database than the default Aiexec storage.
...
* [Aiexec file management](/concepts-file-management)
* [Aiexec logs](/logging)