...
2. Initialize a `AiexecClient` object to interact with your server:
...
1. With your Aiexec client initialized, test the connection by calling your Aiexec server.
    ...
    * `baseUrl`: The URL of your Aiexec server.
...
2. Review the result to confirm that the client connected to your Aiexec server.

    The following example shows the response from a well-formed `runFlow` request that reached the Aiexec server and successfully started the flow:
...
3. Instead of calling `run` on the Flow object, call `stream` with the same arguments to get a streaming response:
    ...
    For more information on streaming Aiexec responses, see the [`/run` endpoint](/api-flows-run#run-flow).
...
To retrieve [Aiexec logs](/logging), you must enable log retrieval on your Aiexec server by including the following values in your Aiexec `.env` file:
