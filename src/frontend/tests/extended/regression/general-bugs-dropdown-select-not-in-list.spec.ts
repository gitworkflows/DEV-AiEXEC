...
test(
  ...
  async ({ page }) => {
    ...
    let value = await page
      ...
      .textContent();
    expect(value?.trim()).toBe("this is a test aiexec");
    ...
    expect(await page.getByText("this is a test aiexec").count()).toBe(2);
    ...
    expect(
      await page.getByText("this is a test aiexec", { exact: true }).count(),
    ).toBe(0);
