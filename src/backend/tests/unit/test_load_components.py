...
from lfx.interface.components import aget_all_types_dict, import_aiexec_components
...
class TestComponentLoading:
    ...
    async def test_component_loading_performance_comparison(self, base_components_path):
        ...
        # Both should complete in reasonable time (< 10s for aiexec, < 20s for all_types)
        assert aiexec_duration < 10.0, f"get_aiexec_components_list took too long: {aiexec_duration}s"
