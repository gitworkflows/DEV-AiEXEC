...
from aiexec.services.deps import get_variable_service, session_scope
from aiexec.utils.voice_utils import BYTES_PER_24K_FRAME, VAD_SAMPLE_RATE_16K, resample_24k_to_16k
        ...
        await websocket.send_json(
            {
                "type": "error",
                "code": "aiexec_auth",
                "message": "You must pass a valid Aiexec token or cookie.",
            }
        )
        return None, None
                    ...
                    while True:
                        ...
                        if msg.get("type") == "input_audio_buffer.append":
                            ...
                            if num_audio_samples > AUDIO_SAMPLE_THRESHOLD:
                                ...
                                num_audio_samples = 0
                        elif msg.get("type") == "aiexec.voice_mode.config":
                            await logger.ainfo(f"aiexec.voice_mode.config {msg}")
                            voice_config.progress_enabled = msg.get("progress_enabled", True)
                        elif msg.get("type") == "aiexec.elevenlabs.config":
                            await logger.ainfo(f"aiexec.elevenlabs.config {msg}")
                ...
                try:
                    while True:
                        ...
                        if event.get("type") == "input_audio_buffer.append":
                            ...
                            openai_send(event)
                        elif event.get("type") == "aiexec.elevenlabs.config":
                            await logger.ainfo(f"aiexec.elevenlabs.config {event}")
