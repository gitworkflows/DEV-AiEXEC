...
if TYPE_CHECKING:
    ...
    from aiexec.services.store.schema import StoreComponentCreate
...
async def check_aiexec_version(component: StoreComponentCreate) -> None:
    ...
    aiexec_version = await get_lf_version_from_pypi()
    if aiexec_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Aiexec")
    if aiexec_version != component.last_tested_version:
        await logger.awarning(
            f"Your version of Aiexec ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({aiexec_version}) and try again."
        )
