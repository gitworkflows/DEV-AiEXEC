...
from aiexec.utils.version import fetch_latest_version, get_version_info
from aiexec.utils.version import is_pre_release as aiexec_is_pre_release
...
try:
    ...
    lfx_app = typer.Typer(name="lfx", help="Aiexec Executor commands")
...
def run(
    *,
    ...
    env_log_level = os.environ.get("AIEXEC_LOG_LEVEL")
    if env_log_level is None:
        os.environ["AIEXEC_LOG_LEVEL"] = log_level_str
    else:
        os.environ["AIEXEC_LOG_LEVEL"] = env_log_level.lower()
    ...
    progress = create_aiexec_progress(verbose=verbose)

    # Step 0: Initializing Aiexec
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            ...
            server = AiexecApplication(app, options)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "aiexec")
        'A new version of aiexec is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=aiexec_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if aiexec_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    # Use aiexec.utils.version to get the version info
    version_info = get_version_info()
    aiexec_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= aiexec_is_pre_release(aiexec_version)  # Update pre-release status

    notice = build_version_notice(aiexec_version, package_name)
    ...
    telemetry_text = (
        ...
        if os.getenv("DO_NOT_TRACK", os.getenv("AIEXEC_DO_NOT_TRACK", "False")).lower() != "true"
        else (
            "We are [bold]not[/bold] collecting anonymous usage data to improve Aiexec.\n"
            "To contribute, set: [bold]DO_NOT_TRACK=false[/bold] in your environment."
        )
    )
    access_host = get_best_access_host(host, port)
    access_link = f"[bold]{status_icon} Open Aiexec {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
...
def superuser(
    username: str = typer.Option(
        None, help="Username for the superuser. Defaults to 'aiexec' when AUTO_LOGIN is enabled."
    ),
    password: str = typer.Option(
        None, help="Password for the superuser. Defaults to 'aiexec' when AUTO_LOGIN is enabled."
    ),
    log_level: str = typer.Option("error", help="Logging level.", envvar="AIEXEC_LOG_LEVEL"),
    auth_token: str = typer.Option(
        None, help="Authentication token of existing superuser.", envvar="AIEXEC_SUPERUSER_TOKEN"
    ),
) -> None:
    """Create a superuser.
...
async def _create_superuser(username: str, password: str, auth_token: str | None):
    ...
    if not settings_service.auth_settings.ENABLE_SUPERUSER_CLI:
        ...
        typer.echo("Set AIEXEC_ENABLE_SUPERUSER_CLI=true to enable this feature.")
        raise typer.Exit(1)
    ...
    if settings_service.auth_settings.AUTO_LOGIN:
        if not is_first_setup:
            ...
            typer.echo("1. Set AIEXEC_AUTO_LOGIN=false")
    ...
    # AUTO_LOGIN is false - production mode
        ...
        if not auth_token:
            ...
            typer.echo("To get a token, use: `uv run aiexec api_key`")
            raise typer.Exit(1)
...
# command to copy the aiexec database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'aiexec.db' and 'aiexec-pre.db' files from the cache directory to the current
    directory.
