...
from aiexec.api import health_check_router, log_router, router
from aiexec.api.v1.mcp_projects import init_mcp_servers
...
from aiexec.middleware import ContentSizeLimitMiddleware
from aiexec.services.deps import get_queue_service, get_settings_service, get_telemetry_service
from aiexec.services.utils import initialize_services, initialize_settings_service, teardown_services
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            await logger.adebug(f"Starting Aiexec v{version}...")
        else:
            await logger.adebug("Starting Aiexec...")
        ...
        try:
            ...
            lock_file = Path(tempfile.gettempdir()) / "aiexec_starter_projects.lock"
            ...
            if "aiexec migration --fix" not in str(exc):
            ...
            from aiexec.__main__ import get_number_of_workers
            from aiexec.cli.progress import create_aiexec_shutdown_progress

            log_level = os.getenv("AIEXEC_LOG_LEVEL", "info").lower()
            num_workers = get_number_of_workers(get_settings_service().settings.workers)
            shutdown_progress = create_aiexec_shutdown_progress(
            ...
            try:
                ...
                with shutdown_progress.step(4):
                    await logger.adebug("Aiexec shutdown complete")
...
def create_app():
    ...
    if prome_port_str := os.environ.get("AIEXEC_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from aiexec.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Aiexec's main.py file."""
